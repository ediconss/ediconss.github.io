...
function choose_init_method(callback) {
    var choices = [{name: "自己选择", choice: "user", txtpos_x: -13.2},
                   {name: "随机", choice: "random", txtpos_x: -3.2},
                   {name: "离中心最远", choice: "farthest", txtpos_x: 6.2}
                  ];

    var title = {text: "你可以选择以何种方式分配质心", x: -14, y: 9};

    display_choice(choices, title, callback);
}

function changeData(){
    restart($("#methedset").val(),$("#dataset").val())
}

function changeMethed(){
    restart($("#methedset").val(),$("#dataset").val())
}
...
function reassign_points() {
    ...
    .each("end", function() {
        d3.select("#next_button").html("更新质心").on("click", update_centroids);
...
function update_centroids() {
    ...
    .each("end", function() {
        draw_voronoi();
        d3.select("#next_button").html("重新分配")
...
function add_go_button() {
    ...
    var next_button= d3.select("#button_area").append("div")
    .attr("id", "next_button")
...
function add_next_centroid_button(callback) {
   ...
   var btn= d3.select("#button_area").append("div")
    .attr("id", "next_centroid")
    .attr("name", "nextCentroidButton")
    .attr("class","button").html("添加质心")
    .on("click", callback);
}
